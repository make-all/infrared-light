...
async def async_setup_entry(hass, config_entry, async_add_entities):
    async_add_entities([InfraredLightEntity(config_entry)])
...
class InfraredLightEntity(LightEntity, RestoreEntity):
    ...
    def __init__(self, config_entry) -> None:
        ...
        codes = commands.get("codes", {})
        if not codes:
            raise AttributeError("Config is missing command codes")
        self._default_device = commands.get("device")
        self._default_type = commands.get("type")
        for cmd in codes:
            name = cmd.get("name")
            if not name:
